...
for(n in 1:length(S.full)){
  ...
  func1.extreme.small <- list(stacked.summary, model.sums, weights, theta0, data.summary, funnel.plots, big_k)
  names(func1.extreme.small) <- c("stacked", "models", "weights", "theta0", "data.summary", "funnel.plots", "big.k")
  saveRDS(func1.extreme.small, file = here("R", "Results", paste("func1.extreme.small.", S.target[n], ".rds", sep = "")))
