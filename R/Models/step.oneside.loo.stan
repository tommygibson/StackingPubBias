...
// Did I misspecify the weighted normal lpdf?
...
functions {
  ...
  real weightednormal_lpdf(real y, real sigma, real p, real theta, real tau, vector omega, vector steps) {
    ...
    for(t in 1:T){
      // inv_Phi(steps[t] / 2) would be for 2-sided p-values
      crits[t] = (fabs(inv_Phi(steps[t])) * sigma - theta) / tot_sd; // define std normal quantiles associated with steps
    }
...
transformed data {
  ...
  for(i in 1:S){
    dev[i] = y[i] / s[i];
    p[i] = (1 - exp(normal_lcdf(dev[i] | 0, 1))); // one-sided p-values
  }
  for(i in 1:H){
    dev_h[i] = y_h[i] / s_h[i];
    p_h[i] = (1 - exp(normal_lcdf(dev_h[i] | 0, 1)));
  }
}
