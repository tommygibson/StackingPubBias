### Stacking simulation 3:
### Selection function 1, extreme bias
...
weights <- matrix(nrow = M, ncol = 8) # weight for each model (mav, bai, step functions)
stacked.summary <- matrix(nrow = M, ncol = 4) # mean, sd, 2.5, 97.5
model.sums <- matrix(nrow = 9 * M, ncol = 6) # model, iteration, mean, sd, 2.5%, 97.5%
data.summary <- matrix(nrow = M, ncol = 5) # num. studies, mean.select, mean.full, sd.select, sd.full
funnel.plots <- list()
seeds <- list()
big_k <- matrix(nrow = M, ncol = 8)


for(j in 1:M){
  
  # dat contains simulated y and z
  dat <- matrix(nrow = S.full[1], ncol = 3)
  # standard errors
  s <- runif(S.full[1], 0.1, 0.8)
  # true study-level effects
  theta <- rnorm(S.full[1], mean = theta0, sd = tau)
...
# naming convention: sim.[function].[sample size].[mean (small/large)]
sim.extreme.small.15 <- list(stacked.summary, model.sums, weights, theta0, data.summary, funnel.plots, big_k)
names(sim.extreme.small.15) <- c("stacked", "models", "weights", "theta0", "data.summary", "funnel.plots", "big.k")
saveRDS(sim.extreme.small.15, file = here("R", "Results", "sim.extreme.small.15.rds"))

for(j in 1:M){
  
  # dat contains simulated y and z
  dat <- matrix(nrow = S.full[2], ncol = 3)
  # standard errors
  s <- runif(S.full[2], 0.1, 0.8)
  # true study-level effects
  theta <- rnorm(S.full[2], mean = theta0, sd = tau)
...
sim.extreme.small.30 <- list(stacked.summary, model.sums, weights, theta0, data.summary, funnel.plots, big_k)
names(sim.extreme.small.30) <- c("stacked", "models", "weights", "theta0", "data.summary", "funnel.plots", "big.k")
saveRDS(sim.extreme.small.30, file = here("R", "Results", "sim.extreme.small.30.rds"))
...
for(j in 1:M){
  ...
  # dat contains simulated y and z
  dat <- matrix(nrow = S.full[3], ncol = 3)
  # standard errors
  s <- runif(S.full[3], 0.1, 0.8)
  # true study-level effects
  theta <- rnorm(S.full[3], mean = theta0, sd = tau)
...
sim.extreme.small.60 <- list(stacked.summary, model.sums, weights, theta0, data.summary, funnel.plots, big_k)
names(sim.extreme.small.60) <- c("stacked", "models", "weights", "theta0", "data.summary", "funnel.plots", "big.k")
saveRDS(sim.extreme.small.60, file = here("R", "Results", "sim.extreme.small.60.rds"))
