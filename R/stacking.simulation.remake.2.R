...
S.full <- read_rds(here("R", "sim.1.4.initial.S.rds"))[2,]
...
for(n in 1:length(S.full)){
  ...
  func1.moderate.big <- list(stacked.summary, model.sums, weights, theta0, data.summary, funnel.plots, big_k)
  names(func1.moderate.big) <- c("stacked", "models", "weights", "theta0", "data.summary", "funnel.plots", "big.k")
  saveRDS(func1.moderate.big, file = here("R", "Results", paste("func1.moderate.big.", S.target[n], ".rds", sep = "")))
